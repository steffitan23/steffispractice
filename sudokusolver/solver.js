...
Solver.prototype.backtrack = function (r, c) {
  ...
  if (c > 8) {
    // Moves to next row when end of column is reached
    c = 0;
    r++;
    if (r > 8) {
      // Checks if end of grid is reached
      return true;
    }
  }

  if (this.working_grid[r][c] != 0) {
    // Move to next cell if user has entered a number in current cell
    if (
      !(
        this.validate_row(r, c) &&
        this.validate_column(r, c) &&
        this.validate_box(r, c)
      )
    ) {
      return false;
    }
    ...
    // Goes through all possible numbers if user has left cell blank
    for (var x = 1; x < 10; x++) {
